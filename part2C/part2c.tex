...
where $M$ is the mass of the central mass %at $r=0$ 
...
This will be a good approximation for $r\gg 2M$. Inserting our newfound Taylor expansion into the relativistic \textquote{Doppler} formula we find
...
We know from lecture note 2C that the Schwarzschild radius of a black hole is located at $r=2M$. Since the light we measure is is emitted at a distance $r\approx2M$, it would make sence that there is a black hole at the center of the quasar (light closer to the black hole then $r=2M$ will just \textquote{fall} into it).
...
\scalebox{1.5}{
\begin{tikzpicture}[decoration = {snake,pre length=0pt,post length=0pt,amplitude=0.4mm,segment length=2}]
\draw (0,0) -- (60:4cm) node [pos=0.5,left] {A};
\draw (0,0) -- (10:5cm) node [pos=0.5,below] {B};
\draw (10:5) -- (60:4) node [pos=0.5,right] {C};
\draw[variable=\x, domain=10:60, smooth, samples=60] plot (\x:0.8);
\draw node at (35:1.0) {\(\alpha\)};
\end{tikzpicture}}
